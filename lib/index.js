...
process.listeners('uncaughtException').forEach(function(f, idx) {
	if(!f.toString().match(/YUIDoc/)) ue.push(idx);
});

ue.forEach(function(idx) {
	process.removeListener('uncaughtException', process.listeners('uncaughtException')[idx]);
});
